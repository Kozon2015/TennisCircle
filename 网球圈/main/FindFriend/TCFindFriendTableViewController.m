//
//  TCFindFriendTableViewController.m
//  网球圈
//
...
#import "TCFindFriendTableViewController.h"
...
@interface TCFindFriendTableViewController ()<ActionSheetCustomPickerDelegate>

@property TCFriendTableViewCell *cell;
@property (nonatomic, strong) NSArray *appoints;
@property (nonatomic, strong) NSArray *levelArray;
@property (nonatomic, strong) NSString *level;
@property (nonatomic, strong) NSArray *playingStyleArray;
@property (nonatomic, strong) NSString *playingStyle;
...
@implementation TCFindFriendTableViewController
...
- (IBAction)filter:(UIBarButtonItem *)sender {
    CGPoint point = CGPointMake(self.view.frame.size.width - 85, 64);
    NSArray *titles = @[@"城市",@"水平",@"打法"];
    PopoverView *pop = [[PopoverView alloc] initWithPoint:point titles:titles];
    pop.selectRowAtIndex = ^(NSInteger index){
        if (index==0) {
            ...
            [ActionSheetStringPicker showPickerWithTitle:nil rows:self.levelArray initialSelection:0 doneBlock:^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue) {
                //筛选
                self.level = selectedValue;
                self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(getLevelData)];
                [self.tableView.mj_header beginRefreshing];
            ...
            [ActionSheetStringPicker showPickerWithTitle:nil rows:self.playingStyleArray initialSelection:0 doneBlock:^(ActionSheetStringPicker *picker, NSInteger selectedIndex, id selectedValue) {
                self.playingStyle = selectedValue;
                self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(getPlayingStyleData)];
                [self.tableView.mj_header beginRefreshing];
            } cancelBlock:^(ActionSheetStringPicker *picker) {
                NSLog(@"Block Picker Canceled");
            } origin:self.view];
...
- (void)getPlayingStyleData {
    BmobQuery *query = [BmobQuery queryWithClassName:@"TCFriend"];
    //查询结果
    [query orderByDescending:@"createdAt"];
    [query whereKey:@"playingstyle" equalTo:self.playingStyle];
    [query findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        if (error) {
            if (error.code == 20002) {
                [TCCheckUtil showAlertWithMessage:@"网络故障，请检查一下网络！" delegate:self];
...
- (void)actionSheetPickerDidSucceed:(ActionSheetCustomPicker *)actionSheetPicker origin:(id)origin {
    ...
    self.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(getCityData)];
